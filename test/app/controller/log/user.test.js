...
describe('test/app/controller/log.test.js', () => {
	...
	it('test update projects ', async () => {
		...
		const res = await app
			...
			.post(`${baseUrl}/update`)
			.type('JSON')
			.send({ id: data._id, name: '蔡俊雄', project: 'test' })
			.expect(200)
		assert(res.body.code === 0)
		const new_user = await Project.findById(data._id)
		assert(new_user.name === '蔡俊雄')
		assert(new_user.project === 'test')
	})
})
